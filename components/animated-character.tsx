...
export function AnimatedCharacter() {
  ...
  useEffect(() => {
    ...
    function animateFace() {
      ...
      if (dom.face) gsap.to(dom.face, { yPercent: yLow / 30, xPercent: x / 30 })
      if (dom.eye) gsap.to(dom.eye, { yPercent: yHigh / 3, xPercent: x / 2 })
      if (dom.innerFace) gsap.to(dom.innerFace, { yPercent: y / 6, xPercent: x / 8 })
      if (dom.hairFront) gsap.to(dom.hairFront, { yPercent: yHigh / 15, xPercent: x / 22 })
      if (dom.hairBack && dom.shadow)
        gsap.to([dom.hairBack, ...dom.shadow], { yPercent: (yLow / 20) * -1, xPercent: (x / 20) * -1 })
      if (dom.ear) gsap.to(dom.ear, { yPercent: (y / 1.5) * -1, xPercent: (x / 10) * -1 })
      if (dom.eyebrowLeft && dom.eyebrowRight) gsap.to([dom.eyebrowLeft, dom.eyebrowRight], { yPercent: y * 2.5 })
